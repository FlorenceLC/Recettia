...
// ─── STATE ───────────────────────────────────────────────────
let recipes         = JSON.parse(localStorage.getItem('recettai_recipes') || '[]');
let plannerData     = JSON.parse(localStorage.getItem('recettai_planner') || '{}');
...
// ─── API KEY ─────────────────────────────────────────────────
function getApiKey() {
  return localStorage.getItem('recettai_apikey') || '';
}
...
// ─── MISTRAL API CALL ─────────────────────────────────────────
// Mistral supports standard CORS from browsers — no special headers needed beyond Authorization.
async function callMistral(prompt) {
  ...
  if (!response.ok) {
    ...
    const msg = err?.message || err?.error?.message || `Erreur HTTP ${response.status}`;
    throw new Error(msg);
  }
...
function buildPrompt(type, input) {
  let ctx = '';
  if (type === 'video') {
    ctx = `L'utilisateur a fourni ce lien de vidéo de recette : ${input}\nAnalyse l'URL pour déduire la plateforme et génère une recette plausible selon le contexte.`;
  } else if (type === 'web') {
    ctx = `L'utilisateur a fourni ce lien de page web de recette : ${input}\nAnalyse l'URL et génère la recette correspondante.`;
  } else {
    ctx = `L'utilisateur a collé ce texte de recette :\n\n${input}`;
  }

  return `${ctx}

Retourne UNIQUEMENT cet objet JSON (sans markdown, sans backticks, sans texte avant ou après) :

{
  "title": "Titre de la recette",
  "category": "Viande",
  "servings": 4,
  "source": ${type !== 'text' ? `"${input}"` : 'null'},
  "ingredients": [
    {"qty": "200g", "name": "pâtes"},
    {"qty": "2", "name": "tomates"}
  ],
...
- Les étapes doivent être claires et numérotées implicitement
...
async function analyzeRecipe() {
  ...
  try {
    const prompt = buildPrompt(currentTab, input);
    ...
    if (err.message === 'NO_KEY') {
      ...
      showPage('settings');
    } else if (err.message.includes('401') || err.message.toLowerCase().includes('unauthorized') || err.message.toLowerCase().includes('invalid api')) {
      toast('❌ Clé API invalide. Vérifiez votre clé Mistral dans Paramètres.', 'error');
...
function catClass(cat) {
  const map = { Viande:'cat-viande', Poisson:'cat-poisson', Dessert:'cat-dessert', Cocktail:'cat-cocktail', 'Entrée':'cat-entree' };
  return map[cat] || 'cat-entree';
}
...
function handlePhoto(input, zoneId) {
  ...
  reader.onload = e => {
    ...
    zone.dataset.photo = e.target.result;
...
function collectRecipe() {
  ...
  document.querySelectorAll('#result-steps .step-item').forEach(li => {
    const t = li.querySelector('textarea')?.value.trim();
    if (t) steps.push(t);
  });
...
function persistRecipes() {
  localStorage.setItem('recettai_recipes', JSON.stringify(recipes));
}
...
function renderRecipes() {
  ...
  if (!filtered.length) {
    grid.innerHTML = `<div class="empty-state">
      ...
      <h3>Aucune recette ici</h3>
      <p>Analysez votre première recette pour commencer !</p>
    </div>`;
    return;
  }
...
function buildViewHTML(r) {
  const ings  = (r.ingredients || []).map(i =>
    `<li class="ingredient-item"><span style="color:var(--gold);font-size:.6rem;flex-shrink:0">●</span>
     <span style="font-size:.9rem">${esc(i.qty)} ${esc(i.name)}</span></li>`).join('');
  const steps = (r.steps || []).map((s, i) =>
    `<li class="step-item"><span class="step-num">${i+1}</span>
     <span style="font-size:.9rem;line-height:1.5">${esc(s)}</span></li>`).join('');

  return `
    <div style="background:var(--ink);padding:1.5rem;border-radius:var(--radius-sm);color:white;margin:-1.5rem -1.5rem 1.5rem">
      ...
      <div style="font-size:.85rem;opacity:.7">👥 ${r.servings} personnes</div>
    </div>
...
function renderShoppingSelector() {
  ...
  list.innerHTML = recipes.map(r => `
    <div class="selector-item" onclick="toggleShopping(${r.id},this)">
      ...
      <div>
        <div class="selector-item-title">${esc(r.title)}</div>
        <div class="selector-item-cat">${r.category} · ${r.servings} pers.</div>
      </div>
    </div>`).join('');
}
...
function generateShoppingList() {
  ...
  document.getElementById('shopping-list-result').innerHTML =
    Object.entries(cats).map(([cat, items]) => `
      <div class="shopping-cat-section">
        ...
        ${items.map(item => `
          <div class="shopping-item">
            ...
            <label>${item.qtys.length ? item.qtys.join(' + ') + ' ' : ''}${esc(item.name)}</label>
          </div>`).join('')}
      </div>`).join('');
  ...
  list.innerHTML = recipes.length
    ? recipes.map(r => `
        <div class="recipe-select-item" onclick="addToPlanner(${r.id})">
          ...
          <div>
            <div style="font-size:.9rem;font-weight:500">${esc(r.title)}</div>
            <div style="font-size:.78rem;color:var(--muted)">${esc(r.category)}</div>
          </div>
        </div>`).join('')
...
function printRecipe(r) {
  ...
  win.document.write(`<!DOCTYPE html><html lang="fr"><head>
    <meta charset="UTF-8"><title>${esc(r.title)}</title>
    ...
    <style>
      body{font-family:'DM Sans',sans-serif;max-width:800px;margin:0 auto;padding:2rem;color:#1A1208}
      h1{font-family:'Playfair Display',serif;font-size:2rem;margin-bottom:.4rem}
      h2{font-family:'Playfair Display',serif;font-size:1.2rem;margin:1.4rem 0 .7rem;border-bottom:2px solid #F0D080;padding-bottom:.3rem}
      .meta{color:#8C7B68;font-size:.9rem;margin-bottom:1rem}
      ul{list-style:none;padding:0}li{padding:4px 0;font-size:.92rem}
      img{max-width:300px;border-radius:8px;margin:1rem 0;display:block}
      .cat{display:inline-block;padding:3px 10px;border-radius:99px;font-size:.75rem;font-weight:700;background:#C4541A;color:white;margin-bottom:.7rem}
      a{color:#C4541A}
...
function importJSON(input) {
  ...
  reader.onload = e => {
    try {
      ...
      recipes = [...recipes, ...newOnes];
      persistRecipes();
      updateBadge();
      toast(`✅ ${newOnes.length} recette(s) importée(s).`, 'success');
